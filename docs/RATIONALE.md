# RATIONALE — the long form behind `CLAUDE.md`

`CLAUDE.md` is read in full at the start of every session, so it holds the
invariants and one sentence of why. This file holds everything else: the
measurements a decision was taken against, the account of how a defect was
found, superseded numbers, and the reasoning that is worth having but is not
needed before every edit.

Nothing here was deleted from `CLAUDE.md` — it was moved. The text below is
**verbatim** from the version of `CLAUDE.md` that preceded the trim, regrouped
under headings so `CLAUDE.md` can link to a section. Where a number is stale as
a baseline, it is stale here too; see `docs/CAPABILITIES.md` for the figures
that are maintained as current, and re-measure before quoting any of these.

`docs/DECISIONS.md` remains a pointer to `CLAUDE.md`, which remains the
authority on what must not be reversed. This file is the reasoning, not the
rule.

## Contents

* [1. Evidence stores, routing and the negative-evidence pass](#1-evidence-stores-routing-and-the-negative-evidence-pass)
* [2. Claim verification — the two axes](#2-claim-verification-the-two-axes)
* [3. Biomarkers, the marker vocabulary and the negation grammar](#3-biomarkers-the-marker-vocabulary-and-the-negation-grammar)
* [4. Ranking and capped samples](#4-ranking-and-capped-samples)
* [5. Coverage statements and ingest completeness](#5-coverage-statements-and-ingest-completeness)
* [6. The marker-matcher fixes and the census prefilter](#6-the-marker-matcher-fixes-and-the-census-prefilter)
* [7. Read-only serving and the public surface](#7-read-only-serving-and-the-public-surface)
* [8. Retry against a flaky registry](#8-retry-against-a-flaky-registry)
* [9. Population selection — query sets, not condition substrings](#9-population-selection-query-sets-not-condition-substrings)
* [10. Drug and agent name matching](#10-drug-and-agent-name-matching)
* [11. The remaining LIKE audit and the stopped sweep](#11-the-remaining-like-audit-and-the-stopped-sweep)
* [12. Retrieval relevance and the score floor](#12-retrieval-relevance-and-the-score-floor)
* [13. Provider failure, staleness and request logging](#13-provider-failure-staleness-and-request-logging)
* [14. Schema refusal, shared tables and PDF overflow](#14-schema-refusal-shared-tables-and-pdf-overflow)
* [15. openFDA — devices, drugs, and the four bulk sources](#15-openfda-devices-drugs-and-the-four-bulk-sources)
* [16. FDA negative evidence, the consent gate and validation](#16-fda-negative-evidence-the-consent-gate-and-validation)
* [17. Storage — JSONL, the corpus, secrets](#17-storage-jsonl-the-corpus-secrets)
* [18. The annotated module layout](#18-the-annotated-module-layout)
* [19. The conventions, with their worked examples](#19-the-conventions-with-their-worked-examples)
...
## 1. Evidence stores, routing and the negative-evidence pass
...
## 2. Claim verification — the two axes
...
## 3. Biomarkers, the marker vocabulary and the negation grammar
...
## 5. Coverage statements and ingest completeness
...
## 6. The marker-matcher fixes and the census prefilter
...
## 7. Read-only serving and the public surface
...
## 8. Retry against a flaky registry
...
## 9. Population selection — query sets, not condition substrings
...
## 10. Drug and agent name matching
...
## 11. The remaining LIKE audit and the stopped sweep
...
## 12. Retrieval relevance and the score floor
...
## 13. Provider failure, staleness and request logging
...
## 14. Schema refusal, shared tables and PDF overflow
...
## 15. openFDA — devices, drugs, and the four bulk sources
...
## 16. FDA negative evidence, the consent gate and validation
...
## 17. Storage — JSONL, the corpus, secrets
...
## 18. The annotated module layout
...
## 19. The conventions, with their worked examples
...
## 20. Known-unverified — what has never run for real
...
## 21. Deliberately not built, and the complete FDA surface
...
## 22. Device-parity decisions, recorded 13 August 2026
...
## Appendix — where each decision went

`CLAUDE.md` held **135** paragraph-leading decisions in its
"Decisions that must not be quietly reversed" section before the trim. Every one
has a destination below, named individually. Counts, reconciled rather than
asserted:

| | count |
|---|---|
| decisions in the pre-trim `CLAUDE.md` | **135** |
| stated (condensed) in `CLAUDE.md` today, and held in full here | **119** |
| moved here only — measurement, history, or an instance of a rule that stays | **16** |
| **total accounted for** | **135** |
| of the 135, pinned by a test `CLAUDE.md` names | **45** |

The item titles below are read from the pre-trim file itself, not transcribed,
so the 135 rows and the 135 decisions are the same list by
construction. Everything in the "Decisions" section is also present in this file
**verbatim** — the trim was a move, so no row can be a deletion.

Outside that section, and likewise accounted for: the module layout (§18), the
nine conventions with their worked examples (§19, rules restated in `CLAUDE.md`),
the six known-unverified items (§20, summarised in `CLAUDE.md`), the
deliberately-not-built list and the complete FDA surface (§21), and the five
device-parity decisions of 13 August 2026 (§22, whose four rules are restated in
`CLAUDE.md`). "What this is", "Running it" and "The one file worth changing
first" stayed in `CLAUDE.md` unchanged.

| # | decision (as it was titled) | destination | test / note |
|---|---|---|---|
| 1 | Trial records are NOT in the vector index. | stays + RATIONALE §1 | test_the_index_build_path_never_embeds_a_trial_record (new) |
| 2 | The router keeps a rule-based fallback. | stays + RATIONALE §1 | test_router_falls_back_on_bad_model_output |
| 3 | The negative-evidence pass has two halves that do different jobs. | stays + RATIONALE §1 | test_the_contradiction_prompt_still_permits_an_empty_findings_list (new) |
| 4 | Stopped-trial lookup ORs intervention and indication, never ANDs them. | stays + RATIONALE §1 | test_stopped_trial_in_other_indication_is_not_hidden |
| 5 | The two arms have separate budgets and separate denominators, because they answer different questions. | stays + RATIONALE §1 | test_which_rows_are_shown_does_not_depend_on_the_candidate_window |
| 6 | "Not assessed" and "nothing found" are kept distinct everywhere. | stays + RATIONALE §1 | test_the_master_rule_is_still_stated_in_claude_md (new) — promoted to its own section |
| 7 | Claim verification scores two orthogonal axes, never one. | stays + RATIONALE §2 | test_the_support_and_independence_vocabularies_never_re_merge (new); test_support_and_independence_are_orthogonal |
| 8 | Independence values: COMPANY-LINKED / NO DISCLOSURE / INDEPENDENT / MIXED / N/A — and absence of a disclosure is never independence. | stays + RATIONALE §2 | test_no_disclosure_is_the_honest_default; test_independent_requires_positive_evidence |
| 9 | The company link is judged from a document-level disclosure, not the cited chunk. | stays + RATIONALE §2 | test_disclosure_blob_catches_company_when_cited_chunk_does_not |
| 10 | Claim verification keeps NOT FOUND and CONTRADICTED strictly apart. | stays + RATIONALE §2 | test_empty_retrieval_is_not_found_without_calling_the_model; test_not_found_never_becomes_contradicted |
| 11 | NOT VERIFIABLE is decided at extraction, not verification, and never silently discarded. | stays + RATIONALE §2 | test_not_verifiable_verdict_is_recorded_not_dropped |
| 12 | The numeric downgrade is a deterministic overlay, not a model output. | stays + RATIONALE §2 | test_numeric_mismatch_downgrades_to_partial |
| 13 | Biomarker matching never silently drops a trial, and always shows its evidence. | stays + RATIONALE §3 |  |
| 14 | ELIGIBLE BY EXCLUSION exists because "excludes the opposite" used to have no representation at all. | stays + RATIONALE §3 |  |
| 15 | The vocabulary and negation grammar are shared, in `markers.py`; the conflict-resolution POLICY is deliberately not. | stays + RATIONALE §3 | test_the_two_modules_never_reach_opposite_conclusions |
| 16 | Negation is now recognised at a distance and as a suffix, not just immediately before the marker. | stays + RATIONALE §3 | test_wild_type_never_requires_the_marker_for_every_curated_marker |
| 17 | A marker restated twice in one sentence is one statement, not two — negation is decided once per (sentence, key), not once per regex match. | stays + RATIONALE §3 | test_a_marker_named_twice_in_one_sentence_is_not_a_self_contradiction |
| 18 | A sentence that mandates a TEST is not a sentence that states a RESULT. | stays + RATIONALE §3 |  |
| 19 | A curated marker's verdict and an uncurated guess must never look equally confident. | stays + RATIONALE §3 |  |
| 20 | Two more registry fields were being fetched and silently dropped — `detailed_description` and `keywords`. | stays + RATIONALE §3 |  |
| 21 | A biomarker filter can now OR multiple statuses for one marker; every question-set filter was audited for the old three-state assumption. | stays + RATIONALE §3 |  |
| 22 | The fixed section's OWN denominator is not the same population as the trial-landscape PAGE's, and that gap is a different, pre-existing, deliberate filter — not a residual bug. | RATIONALE §3 | editorial-call pointer kept in CLAUDE.md; the numbers are here |
| 23 | Which trials print in an aggregate section's sample is a deterministic, explainable relevance score, not "recruiting first, then completion date." | stays + RATIONALE §4 |  |
| 24 | The weights were written down, and justified on principle, BEFORE the six ground-truth trials were checked against them — read `config/ranking.yaml`'s header for the full reasoning. | stays + RATIONALE §4 | test_shipped_config_never_scores_sponsor_class; test_shipped_config_weights_phase_and_status_above_the_secondary_signals — the ground-truth measurement moved here |
| 25 | `allocation` (randomised vs not) was fetched and silently dropped, same gap class as `detailed_description` and `keywords`. | stays + RATIONALE §4 |  |
| 26 | The k=30 sample cap and the RECRUITING/NOT_YET_RECRUITING status filter on the `mss-required` question are editorial decisions, left untouched. | stays + RATIONALE §4 | editorial-call pointer; the numbers are here |
| 27 | Biomarker state is a labelled column on the patient landscape, not a sort key — the grouping was cancelling the ranking. | stays + RATIONALE §4 | the old-vs-new rank table moved here |
| 28 | The patient landscape caps what it prints at 30, in `build_landscape`, and says what the cap held back — including which states. | stays + RATIONALE §4 | test_the_default_cap_matches_the_diligence_memos_sample_cap |
| 29 | Every landscape output states its own coverage, on the page, not in a footnote — `coverage.py`, one render function, three surfaces. | stays + RATIONALE §5 |  |
| 30 | The biomarker breakdown line ("N explicit, M by exclusion, K by synonym") required a new axis the matchers didn't track: WHICH pattern fired. | stays + RATIONALE §5 | the not-a-third-value rule stays; the canonical/basis mechanism is here |
| 31 | The breakdown line's population is the section's OWN population, not the query set's — reusing `by_biomarker[marker]` here was a real bug caught before shipping. | stays + RATIONALE §5 | test_section_narrowed_by_status_gets_a_breakdown_scoped_to_its_own_population |
| 32 | The trial fetch runs to exhaustion and asserts against the registry's own total. | stays + RATIONALE §5 |  |
| 33 | `IncompleteFetch` cannot fire when the PROCESS dies, so the ingest writes an in-progress marker before the first network call. | stays + RATIONALE §5 |  |
| 34 | This also closed a hole that had nothing to do with crashing. | stays + RATIONALE §5 | folded into verify_ingest + --max-records |
| 35 | `resolve_marker` substring-matched a query onto the WRONG marker, and it was live on a patient-facing page. | stays + RATIONALE §6 | test_no_query_resolves_to_a_marker_that_does_not_list_it_exactly |
| 36 | A sentence that ENUMERATES an assay panel is not a sentence stating a result. | stays + RATIONALE §6 |  |
| 37 | The biomarker census is a DERIVED column, so a matcher change forces a schema bump. | stays + RATIONALE §6 |  |
| 38 | Query-set membership is an indexed join table, and the token column was DELETED rather than kept beside it. | stays + RATIONALE §6 |  |
| 39 | The census prefilter shipped only after being PROVEN equivalent, and the proof is kept as a test. | stays + RATIONALE §6 | test_a_self_contradicting_trial_is_admitted_by_both_paths; scripts/check_census_parity.py |
| 40 | The page and the health endpoint disagreed about the snapshot date. | stays + RATIONALE §7 |  |
| 41 | Every store WROTE to its own database just to be OPENED, and that was the blocker for serving this publicly. | stays + RATIONALE §7 |  |
| 42 | Two read modes, because the two requirements are not the same flag. | stays + RATIONALE §7 |  |
| 43 | The concurrency test found a REAL defect, and it was the inverse of the reported one. | stays + RATIONALE §7 | test_the_writable_path_uses_wal_so_the_test_above_cannot_silently_regress |
| 44 | `MEDRAG_READ_ONLY=1` is a separate flag, STRICTLY STRONGER than offline, and it implies offline rather than the reverse. | stays + RATIONALE §7 | test_read_only_implies_offline_and_drops_the_api_key |
| 45 | No fetch-on-miss anywhere reachable from a read path. | stays + RATIONALE §7 |  |
| 46 | Still unsafe on a public site, recorded rather than left to be discovered. | stays + RATIONALE §7 |  |
| 47 | Transient registry failures are retried, and the retry is built so it can never launder a failure into a success. | RATIONALE §8 | the four retry invariants below stay; the 41x-HTTP-500 measurement is here |
| 48 | What is retried is deliberately narrow: 429, 500, 502, 503, 504, timeouts and dropped connections. Never a 400 or a 404 — those are ANSWERS. | stays + RATIONALE §8 | test_the_retryable_set_is_transient_failures_only |
| 49 | The backoff floors are chosen against the Purple Book incident, not tuned for speed. | stays + RATIONALE §8 | test_the_first_backoff_is_seconds_not_milliseconds |
| 50 | An exhausted retry raises the ORIGINAL exception, and the family still records PARTIAL. | stays + RATIONALE §8 | test_a_retry_that_exhausts_still_raises_rather_than_returning_empty |
| 51 | Retrying is COUNTED and reported, on success as well as on failure — because retry is precisely a mechanism for turning a loud failure into a quiet delay. | stays + RATIONALE §8 | the dir() guard in tests/test_retry.py |
| 52 | One condition string is a sample, not a population — ingest unions a reviewed query set and records which query found each trial. | stays + RATIONALE §9 |  |
| 53 | Basket trials are only partly reachable, and that gap is stated, never inferred from absence. | stays + RATIONALE §9 |  |
| 54 | The fetch defines the population; the local layer never re-narrows on the condition string. | stays + RATIONALE §9 |  |
| 55 | All three consumers now select by query set. | stays + RATIONALE §9 | test_claim_retrieval_selects_the_fetched_population_not_a_condition_substring |
| 56 | A drug name is a structured fact too — `agents.py` is the one matcher, and `LIKE '%<asset>%'` over the interventions array was the same defect in a fourth place. | stays + RATIONALE §10 |  |
| 57 | Aliases expand at QUERY time, deliberately unlike the biomarker census. | stays + RATIONALE §10 |  |
| 58 | AND for selection, OR for the negative sweep — one matcher, two policies, the same split as `biomarker.py` vs `biomarker_gating.py`. | stays + RATIONALE §10 |  |
| 59 | ANDing introduces a new way to return zero, so the collapse names itself. | stays + RATIONALE §10 |  |
| 60 | Hyphens are split BOTH ways, and that was a measured regression, not a guess. | RATIONALE §10 | measured regression; the separator list is reconstructible from _SPLIT |
| 61 | 68 of the 71 remaining "losses" are a precision GAIN: FOLFOXIRI is not FOLFOX. | stays + RATIONALE §10 | test_a_regimen_is_never_expanded_into_its_component_drugs — the 68-of-71 measurement is here |
| 62 | `agents.py` knows nothing about trials, because drugsFDA needs the same matcher. | stays + RATIONALE §10 |  |
| 63 | A schema bump whose new column is DERIVABLE gets a backfill, not a re-fetch. | stays + RATIONALE §10 |  |
| 64 | Remaining `LIKE '%…%'` over free text, all of them, audited 2026-08-05. | RATIONALE §11 | the audit table, dated 2026-08-05 |
| 65 | The stopped sweep: the exemption was wrong, and the measurement replaced the reasoning. | RATIONALE §11 | the measurement that overturned the exemption; the rule is the query_set line |
| 66 | Fixing it made the indication arm 2.4x larger, which is when a shared budget starts hiding the arm that matters — so the arms were split, and the FIRST SPLIT WAS WRONG. | RATIONALE §11 | arm-sizing measurement; the no-spillover rule stays |
| 67 | The within-arm tiebreak turned out to decide more than the budgets did. | stays + RATIONALE §11 | test_which_rows_are_shown_does_not_depend_on_the_candidate_window |
| 68 | The free-text fallback fires only on an empty result, so a partial drop is undetectable. | stays + RATIONALE §11 | stated as a known, unfixed gap |
| 69 | The relevance floor only ever gated LITERATURE, and half the evidence path walked past it. | stays + RATIONALE §12 | tests/test_retrieval_relevance.py |
| 70 | The floor was recalibrated against a real sample, and the two distributions OVERLAP — the previous comment claiming they "separate cleanly" was written from a single query pair and was wrong. | RATIONALE §12 | recalibration measurement, superseded by the table in this section |
| 71 | `competitive-trials` interpolates neither `{asset}` nor `{indication}`, so it embeds to ONE fixed vector and scores an identical 0.436 for every asset in the sample | RATIONALE §12 | the specific defect; the rule is the constant-string line |
| 72 | `memo.warnings` was snapshotted BEFORE the sections ran | stays + RATIONALE §12 |  |
| 73 | `tests/test_retrieval_relevance.py` is the pin, and it exists because nothing tested retrieval QUALITY. | stays + RATIONALE §12 |  |
| 74 | THE FLOOR IS A CHOSEN TRADEOFF, NOT A SEPARATOR — read the distributions before moving it. | stays + RATIONALE §12 |  |
| 75 | The two distributions OVERLAP. | RATIONALE §12 | the distribution table |
| 76 | A question that interpolates neither `{asset}` nor `{indication}` is a constant string, and `competitive-trials` was one. | stays + RATIONALE §12 | test_no_shipped_question_is_a_constant_string |
| 77 | A CONFIGURED provider that refuses is not the same as no provider, and only the second was handled. | stays + RATIONALE §13 |  |
| 78 | Staleness was computed at import, so the refusal only ever fired on a restart. | stays + RATIONALE §13 |  |
| 79 | A 500 left one access-log line and nothing to debug from, and the obvious fix would have leaked. | stays + RATIONALE §13 |  |
| 80 | The trials.db schema is versioned and a stale database is refused. | stays + RATIONALE §14 |  |
| 81 | The claims table and the trial-landscape table share one renderer. | stays + RATIONALE §14 |  |
| 82 | A reportlab table overflows the page two ways, only one of which raises — both are handled in `table_render.py`, once, for all three renderers. | stays + RATIONALE §14 | test_no_renderer_builds_a_table_wider_than_its_own_page |
| 83 | openFDA is a third structured store, matched on product code — never company. | stays + RATIONALE §15 |  |
| 84 | openFDA DRUG coverage is a fourth structured store, matched on active ingredient — and the obvious matching field would have silently dropped 57% of the database. | stays + RATIONALE §15 |  |
| 85 | The matching field is `products[].active_ingredients[].name`, NOT `openfda.generic_name`. | stays + RATIONALE §15 |  |
| 86 | Approval is a SUBMISSION fact, and there are three states, not two. | stays + RATIONALE §15 |  |
| 87 | "Not found in drugsFDA" never renders as "not approved", and that is enforced at the type level. | stays + RATIONALE §15 | test_no_renderer_turns_an_empty_result_into_an_approval_claim |
| 88 | Ingredient matching is `agents.py`, extended not forked. | stays + RATIONALE §15 |  |
| 89 | Matching locally beats matching through the API's query syntax, and the combination case proves it. | RATIONALE §15 | reconnaissance measurement |
| 90 | What is NOT integrated, stated rather than left to be inferred. | RATIONALE §15 | also in docs/CAPABILITIES.md; §21 is the complete list |
| 91 | The approval statement is RENDERED, not written — the model may write around it and may not write it. | stays + RATIONALE §15 | test_a_memo_for_an_investigational_asset_never_implies_non_approval |
| 92 | Wording that shares a substring with the claim it denies is a bug, twice now. | stays + RATIONALE §15 | tests/test_phrasing.py — promoted to the master-rule corollary |
| 93 | Approval is four axes, not a boolean, and each is rendered separately. | stays + RATIONALE §15 | test_the_memo_renders_each_axis_rather_than_a_single_approved_yes |
| 94 | The drug store carries the same coverage / non-coverage / freshness line as everything else. | RATIONALE §15 | an instance of the master rule, which stays |
| 95 | The question set gained `regulatory-status`, phrased as "what does the FDA record show", not "is it approved". | stays + RATIONALE §15 | test_the_shipped_question_set_has_a_question_that_reaches_the_drug_store |
| 96 | A drug citation resolves to an application number, not a generic FDA RECORD. | stays + RATIONALE §15 |  |
| 97 | Premarket APPROVAL is a fourth regulatory fact, and 510(k)-only was hiding Class III devices entirely. | stays + RATIONALE §15 |  |
| 98 | Bulk ingestion is shared infrastructure (`fda/bulk.py`), not a PMA special case. | stays + RATIONALE §15 |  |
| 99 | Freshness for a bulk source is `export_date`, and refresh means re-download — stated, never implied. | stays + RATIONALE §15 |  |
| 100 | The decision codes were read from the FDA data dictionary, and the dictionary and the data disagree. | stays + RATIONALE §15 |  |
| 101 | "Has a PMA record" is not "was approved", the same way "has a drugsFDA record" was not. | stays + RATIONALE §15 |  |
| 102 | A PMA is NOT automatically Class III. | stays + RATIONALE §15 |  |
| 103 | Originals are keyed off `supplement_number`, not `supplement_type` — and getting that wrong overstates the approval base by 128%. | stays + RATIONALE §15 |  |
| 104 | `device/pma` has no `device_name`, and assuming symmetry with the 510(k) path found nothing at all. | stays + RATIONALE §15 | parse_descriptive_name vs parse_asset stays; the 33%-to-39% correction is here |
| 105 | De Novo is flagged, and until now every one of the 482 rendered as a substantial-equivalence finding. | stays + RATIONALE §15 |  |
| 106 | FAERS closes the asymmetry — devices had MAUDE, drugs had nothing — and it is the source most likely to produce a confidently misleading memo. | stays + RATIONALE §15 |  |
| 107 | A count is not a rate, and the guard is in the type as well as the prose. | stays + RATIONALE §15 | test_the_answer_object_exposes_no_rate_and_cannot_compute_one |
| 108 | The wording avoids the nouns it denies — the third time this has bitten. | stays + RATIONALE §15 | promoted to the master-rule corollary |
| 109 | Two concrete facts do more work than the caveats. | RATIONALE §15 | the two worked facts; the render-in-full rule stays |
| 110 | Retrieval is cached aggregates — not a mirror, not live per memo. | stays + RATIONALE §15 |  |
| 111 | Matching ORs the normalised block with the free-text name, because the normalised field alone reports zero for investigational assets. | stays + RATIONALE §15 |  |
| 112 | Undocumented codes are labelled, not printed as bare numbers. | stays + RATIONALE §15 |  |
| 113 | The self-contradicting-caveat lint (`phrasing.py`) — three worked examples was enough. | stays + RATIONALE §15 | tests/test_phrasing.py |
| 114 | The Orange Book's absence problem is the worst in the tool, and it is an APPLICABILITY problem rather than an absence one. | stays + RATIONALE §15 |  |
| 115 | The rendered sentence is "earliest listed protection lapses <date>", never "generics enter <date>". | stays + RATIONALE §15 | test_the_section_says_protection_lapses_never_that_generics_enter |
| 116 | The limits are IN the section, not only here. | stays + RATIONALE §15 |  |
| 117 | Measured shape (openFDA bulk export, 2026-08-06). | RATIONALE §15 | measured shape, 2026-08-06 |
| 118 | Exclusivity codes are CURATED, not sourced, and labelled as such. | stays + RATIONALE §15 |  |
| 119 | The Purple Book is not an openFDA source, and a 404 nearly recorded it as nonexistent. | stays + RATIONALE §15 |  |
| 120 | `bulk.py` gained a delimited mode rather than a Purple Book fork. | stays + RATIONALE §15 |  |
| 121 | The file is TWO sections and only the second is the database. | stays + RATIONALE §15 |  |
| 122 | Exclusivity coverage is the finding here, and it was measured rather than assumed. | RATIONALE §15 | fill rates; the lead-on-what-it-answers-well shaping stays |
| 123 | Three variants of absence, and the third is specific to this source. | stays + RATIONALE §15 |  |
| 124 | Biosimilar entry is not generic entry, and is rendered in a different shape. | stays + RATIONALE §15 |  |
| 125 | A fourth self-contradicting caveat, caught by the test rather than by hand. | RATIONALE §15 | worked example of the lint |
| 126 | FDA recalls and adverse events are a deterministic negative-evidence half, beside stopped trials but on their own memo lines. | stays + RATIONALE §16 |  |
| 127 | The claim verifier does not transmit before a per-run confirmation. | stays + RATIONALE §16 | tests/test_consent_gate.py |
| 128 | Validation runs against the assembled `Evidence` list, not raw passages. | stays + RATIONALE §16 | test_citations_use_the_assembled_evidence_numbering |
| 129 | Default provider is `none`. | stays + RATIONALE §17 | test_default_config_costs_nothing |
| 130 | JSONL is split on newline only, never `str.splitlines()`. | stays + RATIONALE §17 | test_the_stored_data_readers_never_call_str_splitlines (new) |
| 131 | A malformed corpus line is skipped, counted, and quarantined — never silently dropped. | stays + RATIONALE §17 |  |
| 132 | `save_corpus` appends; it does not read the corpus back. | stays + RATIONALE §17 |  |
| 133 | `write_secure`'s temp file is uniquely named. | stays + RATIONALE §17 |  |
| 134 | Fetched abstracts are not thrown away by a local write failure. | stays + RATIONALE §17 |  |
| 135 | `.env` is written 0600 via atomic rename, and the key is kept out of `Config.__repr__`. | stays + RATIONALE §17 | tests/test_privacy.py |
